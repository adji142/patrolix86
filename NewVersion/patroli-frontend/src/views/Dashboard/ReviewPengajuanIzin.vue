          ...
          <table>
            ...
            <tbody>
              ...
              <tr v-else-if="filteredItems.length === 0">
                <td colspan="9" style="text-align: center; color: #9ca3af; padding: 2rem;">Tidak ada data.</td>
              </tr>
              <tr v-for="(item, idx) in paginatedItems" :key="item.id">
                ...
                <td style="white-space: nowrap;">{{ item.TglIzinAwal }} s/d {{ item.TglIzinAkhir }}</td>
                <td style="max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" :title="item.KeteranganIzin">
                  {{ item.KeteranganIzin }}
                </td>
      ...
      <div class="card" style="width: 100%; max-width: 480px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
          <h2 class="page-title" style="margin: 0;">
            {{ approveAction === 1 ? '✅ Setujui Izin' : '❌ Tolak Izin' }}
          </h2>
        ...
        <div style="margin-bottom: 1rem; padding: 0.75rem; background: #f9fafb; border-radius: 6px; font-size: 0.875rem;">
          ...
          <div><strong>Periode:</strong> {{ selectedItem?.TglIzinAwal }} s/d {{ selectedItem?.TglIzinAkhir }}</div>
          <div style="margin-top: 0.25rem;"><strong>Keterangan:</strong> {{ selectedItem?.KeteranganIzin }}</div>
        </div>
    ...
    filteredItems() {
      ...
      return this.items.filter(
        (i) =>
          ...
          i.KeteranganIzin?.toLowerCase().includes(q),
      );
    },
    totalPages() {
  ...
  methods: {
    async fetchData() {
      ...
      try {
        ...
        const res = await axios.get('/review-pengajuan-izin', { params });
    ...
    async submitApproval() {
      ...
      try {
        await axios.patch(`/review-pengajuan-izin/${this.selectedItem.id}/approve`, {
        ...
        Swal.fire(
          'Berhasil!',
          this.approveAction === 1 ? 'Izin berhasil disetujui.' : 'Izin berhasil ditolak.',
          'success',
        );
      } catch (e) {
    ...
    fotoUrl(fileName) {
      ...
      return `${base}/pengajuan-izin/${fileName}`;
    },
  },
};
</script>