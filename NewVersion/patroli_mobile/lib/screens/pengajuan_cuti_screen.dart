...
import 'package:patroli_mobile/services/cuti_service.dart';
...
class _PengajuanCutiScreenState extends State<PengajuanCutiScreen>
  ...
  Future<void> _fetchKategori() async {
    ...
    try {
      final result = await _cutiService.fetchKategori(token);
      if (result['success'] == true) {
        final list = (result['data'] as List).map((e) => e.toString()).toList();
        setState(() {
          _kategoriList = list;
          if (list.isNotEmpty) _selectedKategori = list.first;
        });
      }
    } catch (_) {
    } finally {
  ...
  Future<void> _fetchRiwayat() async {
    ...
    try {
      final result = await _cutiService.fetchRiwayat(token);
      if (result['success'] == true) {
        setState(() => _riwayat = result['data'] ?? []);
      }
    } catch (e) {
      _showMsg('Error', 'Gagal memuat riwayat cuti.');
    } finally {
  ...
  Future<void> _submit() async {
    ...
    if (_tglAwal == null || _tglAkhir == null) {
      _showMsg('Peringatan', 'Tanggal cuti harus diisi.');
      return;
    }
    if (_selectedKategori == null) {
      _showMsg('Peringatan', 'Kategori cuti harus dipilih.');
      return;
    }
    ...
    try {
      ...
      final params = {
        'TglCutiAwal': DateFormat('yyyy-MM-dd').format(_tglAwal!),
        'TglCutiAkhir': DateFormat('yyyy-MM-dd').format(_tglAkhir!),
        'KeteranganCuti': _keteranganCtrl.text.trim(),
        'KategoriCuti': _selectedKategori!,
        if (fotos.isNotEmpty) 'Fotos': fotos,
      };

      debugPrint('[PengajuanCuti] submit params: $params');

      final result = await _cutiService.submit(token, params);
      if (result['success'] == true) {
        _resetForm();
        _fetchRiwayat();
        if (mounted) {
      ...
      debugPrint('[PengajuanCuti] submit error: $e');
      debugPrint('[PengajuanCuti] stack trace: $st');
  ...
  void _resetForm() {
    ...
    setState(() {
      ...
      _tglAkhir = null;
      _fotoFiles.clear();
      if (_kategoriList.isNotEmpty) _selectedKategori = _kategoriList.first;
    });
  }
  ...
  Widget _approvalBadge(int approval) {
    ...
    final colors = {0: Colors.orange, 1: Colors.green, 2: Colors.red};
    return Container(
                ...
                children: [
                  ...
                  _isLoadingKategori
                      ...
                      : DropdownButtonFormField<String>(
                          ...
                          validator: (v) => v == null ? 'Pilih kategori cuti' : null,
                        ),
                ],
              ),
            ),
    ...
    if (_riwayat.isEmpty) {
      return Center(
        child: Column(
          ...
          children: [
            Icon(Icons.beach_access_outlined, size: 60, color: Colors.grey[300]),
            const SizedBox(height: 12),
            Text(
  ...
  Widget _buildRiwayatCard(Map<String, dynamic> item) {
    final tglAwal = item['TglCutiAwal'] ?? '-';
    final tglAkhir = item['TglCutiAkhir'] ?? '-';
    final keterangan = item['KeteranganCuti'] ?? '-';
    final kategori = item['KategoriCuti'] ?? '-';
            ...
            SizedBox(
              ...
              child: ListView.separated(
                ...
                itemBuilder: (ctx, i) => ClipRRect(
                  ...
                  child: Image.network(
                    '${AppConfig.baseUrl.replaceAll('/api', '')}/pengajuan-cuti/${fotos[i]['FileName']}',
                    width: 60,
                    height: 60,
                    fit: BoxFit.cover,
